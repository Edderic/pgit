...
describe 'PGit::Installer::Configuration' do
  ...
  describe '~/pgit.rc.yml does not exist' do
    it 'should ask to continue or not' do
      ...
      allow(File).to receive(:open).with(expanded_path, 'w').and_yield(fake_writable_file).and_return(fake_writable_file)
      allow(YAML).to receive(:dump).with(PGit::Configuration.default_options, fake_writable_file)
